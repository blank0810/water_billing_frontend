...
export default function UserManagementTable(props: { tableData: RowObj[] }) {
  ...
  const [userToDelete, setUserToDelete] = React.useState<RowObj | null>(null);

  const handleEditClick = (user: RowObj) => {
  ...
  const openDeleteModal = (user: RowObj) => {
  ...
  const handleDeleteUser = async (userId: number | string) => {
    try {
      console.log(`You deleted ${userId}`);
    } catch (error) {
  ...
  const [userToView, setUserToView] = React.useState<RowObj | null>(null);

  const openViewModal = (user: RowObj) => {
    setUserToView(user);
    setIsViewOpen(true);
  };
  ...
  const openUpdateConfirmModal = (user: RowObj) => {
    setUpdatedUser(user);
    setIsUpdateModalOpen(true);
  };
  ...
  const columns = [
    ...
    columnHelper.display({
      ...
      cell: (info) => (
        <Flex gap="10px">
          <Box as="button" onClick={() => openViewModal(info.row.original)}>
            <Icon
              as={MdVisibility}
              w={5}
              h={5}
          ...
          <Box as="button" onClick={() => handleEditClick(info.row.original)}>
            <Icon
              as={MdEdit}
              w={5}
              h={5}
          ...
          <Box as="button" onClick={() => openDeleteModal(info.row.original)}>
            <Icon
              as={MdDelete}
              w={5}
              h={5}
        ...
        >
          ...
          <Flex align="center" gap="12px" flexWrap="wrap">
            ...
            <Button
              ...
              _hover={{ bg: 'gray.200' }}
              onClick={() => {
                setSorting([{ id: 'userType', desc: false }]);
              }}
            >
              <Icon as={MdSort} />
            </Button>
