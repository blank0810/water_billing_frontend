...
interface EditUserDrawerProps {
  isOpen: boolean;
  onClose: () => void;
  userData: any; // Replace with better type if available
...
export default function EditUserDrawer({
  ...
  // Check if password fields are changed (non-empty new password)
  const checkPasswordChanged = () => {
    return !!formData.newPassword;
  };

  const handleSaveClick = () => {
    // Before saving, open modal
    const pwdChanged = checkPasswordChanged();
    setIsPasswordChanged(pwdChanged);
    setIsModalOpen(true);
  };
  ...
  const handleConfirmSave = async () => {
    ...
    try {
      await Promise.resolve(onSave(formData)); // if onSave is async, no need for Promise.resolve

      toast({
        title: 'User updated.',
        description: isPasswordChanged
          ? 'User profile and password were successfully updated.'
          : 'The user information was successfully saved.',
        status: 'success',
        duration: 3000,
        isClosable: true,
        position: "top-right"
              ...
              <>
                ...
                <Flex justify="flex-end" mt={2}>
                  <Link
                    ...
                    cursor="pointer"
                  >
                    {showSecurityForm ? 'Edit profile instead?' : 'Change security details instead?'}
                  </Link>
                </Flex>
              </>
            ) : (
